...
public class DLFileEntryTransactionTest {
	...
	public void testTransactionRollbackUpdateFileInStore() throws Throwable {
		...
		try {
			InputStream fileInputStream = _fileSystemStore.getFileAsStream(
				TestPropsValues.getCompanyId(), _group.getGroupId(),
				_dlFileEntry.getName(), null);

			Assert.assertArrayEquals(
				StreamUtil.toByteArray(fileInputStream), originalBytes);
			...
			TransactionInvokerUtil.invoke(
				...
				() -> {
					_dlFileEntryLocalService.updateFileEntry(
						...
						null, new ByteArrayInputStream(updatedBytes), 0, null,
						null, ServiceContextTestUtil.getServiceContext());
					...
					Assert.assertArrayEquals(
						StreamUtil.toByteArray(updatedFileInputStream),
						updatedBytes);
